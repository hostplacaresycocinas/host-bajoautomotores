...
const CatalogoPage = () => {
  ...
  const fetchMarcas = async () => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/cars/brands?tenant=${TENANT_PUBLIC}`,
        { cache: 'no-store' }
      );
      if (!response.ok) {
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/categories?tenant=${TENANT_PUBLIC}`,
        { cache: 'no-store' }
      );
      if (!response.ok) {
  ...
  const fetchCars = async (
    ...
    setLoading(true);
    try {
      let url = `${API_BASE_URL}/api/cars?page=${page}&limit=${ITEMS_PER_PAGE}&tenant=${TENANT_PUBLIC}`;
